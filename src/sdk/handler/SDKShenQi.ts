class SDKShenQi implements ISDKHandler {
    ...
    private static _instance: SDKShenQi;
    constructor() { }

    private logoUrl: string = "http://cdn.ih5games.com/FREEGAME/logo.png";
    private GAME_TITLE: string = '';
    private GAME_DESC: string = '';
    private productCode = "54691280226976290481495087372468";

    // 没有强制限制，仿一个Singleton
    public static getInstance(): SDKShenQi {
        if (this._instance == null) {
            this._instance = new SDKShenQi();
        }
    ...
    */
    private onGetUserInfo(event: egret.Event): void {
        ...
        if ((info.code == ChannelDefine.SUCCESS) && (info.userid)) {
            ...
            egret.error("SDKQuick.login() failed. Error sign.");
            this.loginFailed();
        }
    }
    ...
    public onCreateRole(loginInfo: ILoginInfo, playerInfo: IPlayerInfo) {
        ...
        // alert("创建时间"+roleInfo.roleCreateTime);
