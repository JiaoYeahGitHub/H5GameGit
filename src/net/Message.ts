...
class Message {
    ...
    private isLogoned: boolean = true;
    public constructor(cmdId: number = 0, isLogoned: boolean = true) {
        this.isLogoned = isLogoned;
        if (cmdId > 0) {
            this.cmdId = cmdId;
    ...
    public setLoginCode(code: number): void {
        if (this.isLogoned) {
    ...
    public pack(): void {
        this.data.setInt32(0, this.posIndex - 4);
    ...
    public getMessage(): ArrayBuffer {
        ...
        if (this.data.byteLength > this.posIndex) {
            var size = this.data.getInt32(this.posIndex);
            buffer = this.msg.slice(this.posIndex, this.posIndex + size);
            this.posIndex += size;
        }
        return buffer;
    }
