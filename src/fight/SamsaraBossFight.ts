class SamsaraBossFight extends BaseFightScene implements IFightScene {
	...
	protected registFightMessage(): void {
		...
		GameDispatcher.getInstance().addEventListener(MESSAGE_ID.SAMSARA_BOSS_OTHERBODY_MSG.toString(), this.onResAddOtherFightBodyMsg, this);
		GameCommon.getInstance().addMsgEventListener(MESSAGE_ID.SAMSARA_BOSS_FIGHT_MSG.toString(), this.onResFightUpdateMsg, this);
		GameCommon.getInstance().addMsgEventListener(MESSAGE_ID.SAMSARA_BOSS_TARGET_MSG.toString(), this.onResBossAttackTargetMsg, this);
		GameCommon.getInstance().addMsgEventListener(MESSAGE_ID.SAMSARA_BOSS_RANK_MSG.toString(), this.onResRankMsg, this);
		GameCommon.getInstance().addMsgEventListener(MESSAGE_ID.SAMSARA_BOSS_REBORN_MSG.toString(), this.onResReborn, this);
		GameCommon.getInstance().addMsgEventListener(MESSAGE_ID.SAMSARA_BOSS_OTHERLEAVE_MSG.toString(), this.onResOhterLeaveMsg, this);
	...
	protected removeFightMessage(): void {
		...
		GameDispatcher.getInstance().removeEventListener(MESSAGE_ID.SAMSARA_BOSS_OTHERBODY_MSG.toString(), this.onResAddOtherFightBodyMsg, this);
		GameCommon.getInstance().removeMsgEventListener(MESSAGE_ID.SAMSARA_BOSS_FIGHT_MSG.toString(), this.onResFightUpdateMsg, this);
		GameCommon.getInstance().removeMsgEventListener(MESSAGE_ID.SAMSARA_BOSS_TARGET_MSG.toString(), this.onResBossAttackTargetMsg, this);
		GameCommon.getInstance().removeMsgEventListener(MESSAGE_ID.SAMSARA_BOSS_RANK_MSG.toString(), this.onResRankMsg, this);
		GameCommon.getInstance().removeMsgEventListener(MESSAGE_ID.SAMSARA_BOSS_REBORN_MSG.toString(), this.onResReborn, this);
		GameCommon.getInstance().removeMsgEventListener(MESSAGE_ID.SAMSARA_BOSS_OTHERLEAVE_MSG.toString(), this.onResOhterLeaveMsg, this);
	...
	public onParseFightMsg(enterMsg: Message): void {
		...
		GameFight.getInstance().samsarabossId = enterMsg.getShort();
		this.bossMaxHp = enterMsg.getLong();
		this.currBossHp = enterMsg.getLong();
		this.beforeBossHp = this.currBossHp;
		var samsarabossModel: Modelboss = JsonModelManager.instance.getModelboss()[GameFight.getInstance().samsarabossId];
		this.onSwitchMap(samsarabossModel.map);
	...
	protected onInitFightInfoBar(): void {
		...
		if (!this.fight_info_bar) {
			...
			this.fight_info_bar.skinName = skins.SamsaraInfoBarSkin;
	...
	protected onStartFight(): void {
		...
		let samsarabossModel: Modelboss = JsonModelManager.instance.getModelboss()[GameFight.getInstance().samsarabossId];
		//初始化信息界面
		let awarditem: AwardItem = GameCommon.parseAwardItem(samsarabossModel.yulan);
		...
		this.monsterBody = this.mainscene.getBodyManager().onCreateMapBody(samsarabossModel.modelId, [mapNode], BODY_TYPE.BOSS) as MonsterBody;
		...
		for (let i: number = 0; i < DataManager.getInstance().dupManager.samsarebossData.otherFightDatas.length; i++) {
			this.onCreateOtherFightBody(DataManager.getInstance().dupManager.samsarebossData.otherFightDatas[i], this.mainscene.mapInfo.getGridNearByNode(mapNode, 2, 2));
		}
		...
		if (GameFight.getInstance().samsaraTargetMsg) {
	...
	private onResFightUpdateMsg(messageEvenet: GameMessageEvent): void {
		...
		if (bossid != GameFight.getInstance().samsarabossId)
			return;
	...
	private onSendFightInfo2Server(): void {
		...
		if (this.fightStatus == FIGHT_STATUS.Fighting) {
			...
			var fightinfoMsg: Message = new Message(MESSAGE_ID.SAMSARA_BOSS_FIGHT_MSG);
			fightinfoMsg.setShort(GameFight.getInstance().samsarabossId);
	...
	private onResBossAttackTargetMsg(): void {
		if (!GameFight.getInstance().samsaraTargetMsg) return;

		var msg: Message = GameFight.getInstance().samsaraTargetMsg;
		GameFight.getInstance().samsaraTargetMsg = null;
		...
		if (this.targetIsSelf) {//判断下目标是不是自己
			...
			if (targetBodys.length == 0) {//如果没有在场景里找到目标对象
				...
				DataManager.getInstance().dupManager.samsarebossData.addOhterFightData(ohterfightData);
			...
			var otherTargetData: OtherFightData = DataManager.getInstance().dupManager.samsarebossData.findOneOtherFightData(targetData.playerId);
	...
	private onOtherLeaveHandler(otherbodys: PlayerBody[], playerId: number): void {
		if (otherbodys.length > 0) {
			...
			DataManager.getInstance().dupManager.samsarebossData.removeOneOhterFightData(playerId);
		}
	}
	/**
	 * BOSS攻击的逻辑
	...
	public onFinishFight(result: number): void {
		...
		if (result == FightDefine.FIGHT_RESULT_FAIL) {
			...
			GameDispatcher.getInstance().dispatchEvent(new egret.Event(GameEvent.MODULE_WINDOW_OPEN_WITH_PARAM), new WindowParam("SamsaraBossKillLogPanel", new SamsaraBossKillLogParam(GameFight.getInstance().samsarabossId, 11, true)));
		}
	...
	public onDestroyScene(): void {
		...
		DataManager.getInstance().dupManager.samsarebossData.removeAllOtherFightData();
		Tool.removeTimer(this.onTimerDown, this, this.timerRunTime);
		// this.monsterBody.removeEventListener(GameEvent.OTHERBODY_ATTACK_EVENT, this.onBossAttack, this);
		this.monsterBody = null;
		GameFight.getInstance().samsarabossId = null;
