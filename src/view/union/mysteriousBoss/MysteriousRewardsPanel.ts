class MysteriousRewardsPanel extends BaseWindowPanel {
	...
	protected onInit(): void {
		...
		this.setTitle("奖励");
		this.bossReawards = JsonModelManager.instance.getModelzhaohuanrewards();
		this.award_groups = [];
		this.items = [];
		for (let i = 0; i < 5; ++i) {
		...
		this.tabBtns = [];
		let level: string[] = ["成功奖励", "失败奖励"];
		for (let i = 0; i < 4; ++i) {
			this.tabBtns[i] = this["tab" + i];
			this.tabBtns[i].value = i;
			if(level[i]){
				(this.tabBtns[i]["tab_name"] as eui.Label).text = level[i];
			} else {
				this.tabBtns[i].visible = false;
			}
		}
	...
	private updateUI() {
		this.updateTabs();
		var bossReaward: Modelzhaohuanrewards = this.bossReawards[this.currIndex];
...
// class MysteriousAwdView extends BaseTabView {
// 	private reward_keys: string[] = ["firstReward", "secondReward", "Reward345", "canyuReward", "killReward"];
// 	constructor(owner: MysteriousRewardsPanel) {
// 		super(owner);
// 	}
// 	protected onInit(): void {
// 		super.onInit();
...
// 		var currIndex: number = (this.owner as MysteriousRewardsPanel).currIndex;
// 		var bossReaward: Modelzhaohuanrewards = JsonModelManager.instance.getModelzhaohuanrewards()[currIndex];
// 		for (var i: number = 0; i < this.reward_keys.length; i++) {
// 			var rewardKeys: string = this.reward_keys[i];
// 			var rewardItems: AwardItem[] = GameCommon.getInstance().onParseAwardItemstr(bossReaward[rewardKeys]);
// 			(this["award_group" + i] as eui.Group).removeChildren();
// 			var item: eui.Group = this["item_" + i] as eui.Group;
// 			if (!rewardItems) {
// 				item.visible = false;
// 				continue;
// 			} else {
// 				item.visible = true;
// 			}
// 			for (var n: number = 0; n < rewardItems.length; n++) {
// 				var waveAward: AwardItem = rewardItems[n];
