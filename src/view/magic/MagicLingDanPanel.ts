class MagicLingDanPanel extends BaseWindowPanel {
	...
	public onShowWithParam(param): void {
		...
		for (let k in models) {
			if (models[k].type == this.blessType && models[k].subtype == 1) {
	...
	private onShowInfo(idx: number): void {
		...
		if (lvNum > 0) {
			str = GameDefine.BlessName[this.blessType] + "全属性:" + (curr.harm * lvNum) + '\n';
			nextStr = GameDefine.BlessName[this.blessType] + '全属性:' + (curr.harm * (lvNum + 1)) + '\n';
		}
		else {
			str = GameDefine.BlessName[this.blessType] + "全属性:0\n";
			nextStr = GameDefine.BlessName[this.blessType] + '全属性:' + curr.harm + '\n';
		}
