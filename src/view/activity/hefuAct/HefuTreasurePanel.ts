class HefuTreasurePanel extends BaseTabView {
	...
	protected onSkinName(): void {
		this.skinName = skins.HefuTreasurePanelSkin;
	}
	protected get manager(): FestivalWuYiManager {
	...
	protected onInit(): void {
		...
		super.onInit();
		this.currHasNum = [];
		this.points[0].register(this.reward_week_btn, new egret.Point(101, -10), DataManager.getInstance().festivalWuYiManager, "checkTreasurePoint");
	...
	protected onRegist(): void {
		...
		for (let i: number = 0; i < 4; i++) {
			this[`award_box_img${i}`].name = (i + 1) + '';
			(this[`award_box_img${i}`] as eui.Group).addEventListener(egret.TouchEvent.TOUCH_TAP, this.onTouchBox, this);
		}
		for (let i = 0; i < 8; i++) {
			if ((this[`btn${i}`] as eui.Group)) {
				(this[`btn${i}`] as eui.Group).name = i + '';
				(this[`btn${i}`] as eui.Group).addEventListener(egret.TouchEvent.TOUCH_TAP, this.onShowAwardTips, this);
			}
		}
		this.examineCD(true);
	...
	protected onRemove(): void {
		...
		for (let i: number = 0; i < 4; i++) {
			(this[`award_box_img${i}`] as eui.Group).removeEventListener(egret.TouchEvent.TOUCH_TAP, this.onTouchBox, this);
		}
		for (let i = 0; i < 8; i++) {
			if ((this[`btn${i}`] as eui.Group)) {
				(this[`btn${i}`] as eui.Group).removeEventListener(egret.TouchEvent.TOUCH_TAP, this.onShowAwardTips, this);
			}
	...
	private onUpdateWeekAward(): void {
		...
		for (let i: number = 0; i < this.WEEK_AWD_MAX; i++) {
			...
			if (this.manager.treasureAwdIdx >= times) {
				curBox = curBox + 1;
				boxImg.source = 'hefuActbox_open_png';
				boxImg.y = -12;
			} else {
				boxImg.y = -0;
				boxImg.source = 'hefuActbox_png';
				if (this.manager.zhuanpanNum >= times) {
					if (!this.reward_week_btn.enabled) {
						this.reward_week_btn.enabled = true;
					}
					GameCommon.getInstance().addAnimation('baoxiangtixing', new egret.Point(-15, -10), animgrp, -1);
					curBox = curBox + 1;
				}
				else {
					// if (curBox == i) {
					...
					// goodsItem.onUpdate(box.type, awardItem.id, 0, awardItem.quality, awardItem.num, awardItem.lv);
					// this.nextNBItemDesc.text = '再抽' + (times - this.manager.zhuanpanNum) + '次可获得';
	...
	private onTouchBox(event: egret.Event): void {
		...
		let weekAwdParams: string[] = this.manager.zhuanpanModel.box.split("#");
		let params: string[];
		// for (let i: number = 0; i < Number(img.name); i++) {
		// 
		params = weekAwdParams[Number(img.name) - 1] ? weekAwdParams[Number(img.name) - 1].split(",") : null;
		// }
		var box: Modelbox = JsonModelManager.instance.getModelbox()[params[1]];
		...
		if (!this.isRun) {
			...
			for (i = 0; i < _rewards.length; i++) {
				if (DataManager.getInstance().festivalWuYiManager.zhuanpanId == _rewards[i].id && DataManager.getInstance().festivalWuYiManager.zhuanpanType == _rewards[i].type && DataManager.getInstance().festivalWuYiManager.zhuanpanAwardNum == _rewards[i].num) {
					...
					egret.Tween.get(this.disc_outside).to({ rotation: RotationLong }, 1700, egret.Ease.sineInOut).call(this.onPlayDone, this).wait(2300).call(this.onReady, this);
					return;
				}
			}
			//前后端配置对不上了
