class Activity666RankPanel extends BaseTabView {
	...
	protected onSkinName(): void {
		this.skinName = skins.Activity666RankPanelSkin;
	}
	private get manager(): Activity666Manager {
		return DataManager.getInstance().a666Manager;
	}
	protected onInit(): void {
		this.scroll.verticalScrollBar.autoVisibility = false;
		this.scroll.verticalScrollBar.visible = false;
		this.itemlist.itemRenderer = Act666RankItem;
		this.itemlist.itemRendererSkinName = skins.Activity666RankItemSkin;
	...
	protected onRegist(): void {
		super.onRegist();
		GameCommon.getInstance().addMsgEventListener(MESSAGE_ID.ACT_666_RANK_MESSAGE.toString(), this.onupdateRanks, this);
		GameDispatcher.getInstance().addEventListener('LABA_666_LOOP_RANK', this.openRankView, this);
		this.examineCD(true);
	}
	protected onRemove(): void {
		super.onRemove();
		GameCommon.getInstance().removeMsgEventListener(MESSAGE_ID.ACT_666_RANK_MESSAGE.toString(), this.onupdateRanks, this);
		GameDispatcher.getInstance().removeEventListener('LABA_666_LOOP_RANK', this.openRankView, this);
	...
	private requestRankInfo(): void {
		let rankMsg: Message = new Message(MESSAGE_ID.ACT_666_RANK_MESSAGE);
	...
	public onupdateRanks(): void {
		...
		let rankJson = JsonModelManager.instance.getModelxianlvpaihang();
		let rankModels: Modelxianlvpaihang[] = [];
...
class Act666RankItem extends BaseListItem {
	private topGroup: eui.Group;
	private imgRankBG: eui.Image;
	...
	protected onUpdate(): void {
		let model: Modelxianlvpaihang = this.data;
		...
		let rankData: CrossServerConsumeRank = DataManager.getInstance().a666Manager.consumeitem_ranks[model.rankMin - 1];
		if (model.rankMin > 3) {
			this.topGroup.visible = false;
			this.otherGroup.visible = true;
			this.imgRankBG.source = "rankbg_4_png";
			this.rank_num_lab.text = model.rankMin + "-" + model.rankMax;
		} else {
			if (rankData) {
				this.topGroup.visible = true;
				this.otherGroup.visible = false;
				this.imgRankBG.source = "rankbg_" + rankData.rank + "_png";
				...
				this.imgRankBG.source = "rankbg_" + model.rankMin + "_png";
	...
	private onLoopRankView(): void {
		GameDispatcher.getInstance().dispatchEvent(new egret.Event('LABA_666_LOOP_RANK'));
	}
}