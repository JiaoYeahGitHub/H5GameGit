class TurnplateAwardPanel extends BaseWindowPanel {
	...
	protected onSkinName(): void {
		this.skinName = skins.TurnplateAwardPanelSkin;
	}
	...
	protected onRefresh(): void {
		...
		if(this.param.titleSource && this.param.titleSource.length>0){
			this.setTitle(this.param.titleSource);
		}else{
			this.basic["title_group"].visible = false;
		}
	...
	private onUpdateAward(): void {
		...
		if (this.param.itemAwards) {
			for (var i: number = 0; i < this.param.itemAwards.length; i++) {
				...
				// goodsInstance.scaleX = goodsInstance.scaleY = 0.8;
