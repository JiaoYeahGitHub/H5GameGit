class PlayerHeadFramePanel extends BaseWindowPanel {
    ...
    private itemList: HeadFrameItem[];
    private currItem: HeadFrameItem;
	...
	protected onInit(): void {
		this.setTitle("更换头像框");
        this.setTitleErji("头像框");
		super.onInit();
		GameCommon.getInstance().addUnderlineStr(this.need);
        this.onInitHead();
		this.onRefresh();
        let redPo = this.createRedPoint();
        redPo.register(this.btn_up, new egret.Point(150, -5), DataManager.getInstance().playerManager, "checkRedPointHeadFrameUp");
	}
    ...
    private onInitHead(){
        ...
        this.itemList = [];
        let list = JsonModelManager.instance.getModeltouxiangkuang();
        for(let k in list){  
			let item = new HeadFrameItem(this, list[k]);
			// this.groupList.addChild(item);
			this.itemList.push(item);
			item.update();
        }
        this.itemList.sort(function(a: HeadFrameItem, b: HeadFrameItem): number {
    ...
    protected onRegist(): void {
		...
		GameCommon.getInstance().addMsgEventListener(MESSAGE_ID.OPT_PLAYER_HEAD_FRAME_UP_MESSAGE.toString(), this.onCallbackUp, this); 
	}
	protected onRemove(): void {
		...
		GameCommon.getInstance().removeMsgEventListener(MESSAGE_ID.OPT_PLAYER_HEAD_FRAME_UP_MESSAGE.toString(), this.onCallbackUp, this); 
	}
    private onTouchUp(){
        let level = DataManager.getInstance().playerManager.player.getHeadFrameLevel(0);
        ...
        if(modelNext){
            ...
            if(GameCommon.getInstance().onCheckItemConsume(rew.id, rew.num)){
                var message: Message = new Message(MESSAGE_ID.OPT_PLAYER_HEAD_FRAME_UP_MESSAGE);
                this.upItem = null;
                message.setByte(0);
    ...
    private onTouchChange() {
        if(this.currItem){
            let player = DataManager.getInstance().playerManager.player;
            if(player.headFrameIndex != this.currItem.model.id){
                var message: Message = new Message(MESSAGE_ID.OPT_PLAYER_HEAD_FRAME_CHANGE_MESSAGE);
    ...
    private onCallbackUp(){
        ...
        if(this.upItem){
            ...
            GameCommon.getInstance().addAnimation('tujianshengji', new egret.Point(320, 390), this.erjiPanel, 1);
        }
    }
    private updateUpPanel(upItem: HeadFrameItem){
        let player = DataManager.getInstance().playerManager.player;
        let model: Modeltouxiangkuang = upItem.model;

        this.playerHeadEJ.setHead(player.headIndex, model.id);

        let level = PlayerHeadFramePanel.getLevel(model.id);
        this.lbLevelEJ.text = level.toString();
        if(PlayerHeadFramePanel.getLevel(model.id) > 0){
    ...
    public setUpItem(upItem: HeadFrameItem){
        this.upItem = upItem;
    ...
    private jihuoMethod(){
        if(this.upItem){
            let player = DataManager.getInstance().playerManager.player;
            let model: Modeltouxiangkuang = this.upItem.model;
    ...
    public static getLevel(id){
        return DataManager.getInstance().playerManager.player.getHeadFrameLevel(id);
    }
    ...
    private updateAttri(player: Player){
        var tempAttribute: number[] = player.getHeadFrameAttribute();
        ...
        let level = player.getHeadFrameLevel(0);
        // this.lbLevel.text = level.toString();
        this.currProTitle.text = level + "级属性";
        this.nextProTitle.text = (level + 1) + "级属性";
        let modelNext: Modeltouxiangkuangshengji = JsonModelManager.instance.getModeltouxiangkuangshengji()[level];
        if(modelNext){
            var tempAttributeNext: number[] = player.getHeadFrameAttribute(modelNext);
            let attRight = "";
            for (let i = 0; i < 4; i++) {
    ...
    private onUpdateHead(player: Player){
        for(let i = 0; i < this.itemList.length; ++i){
            ...
            if(this.itemList[i].model.id == player.headFrameIndex){
                this.setCurrItem(this.itemList[i]);
            }
        }
    }
    public setCurrItem(item: HeadFrameItem){
        if(this.currItem){
...
class HeadFrameItem extends eui.Component {
    private systemOptPanel: PlayerHeadFramePanel;
    ...
    private lbLevel: eui.Label;
    public model: Modeltouxiangkuang;
    private isJingJie: boolean;
    private isCost: boolean;
    constructor(systemOptPanel: PlayerHeadFramePanel, model: Modeltouxiangkuang) {
        super();
    ...
    public update(){
        ...
        this.playerHead.setHead(player.headIndex, this.model.id);
        this.currency.visible = this.lbCount.visible = false;
        this.isJingJie = this.isCost = true;
        let level = PlayerHeadFramePanel.getLevel(this.model.id);
        this.lbLevel.text = level.toString();
    ...
    private onTouchIcon(){
        if(DataManager.getInstance().playerManager.player.getHeadFrameLevel(this.model.id) > 0 || !this.model.cost){
            this.systemOptPanel.setCurrItem(this);
        } else {
            GameCommon.getInstance().addAlert("头像框未激活！");
        }
    }
}