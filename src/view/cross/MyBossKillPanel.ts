class MyBossKillPanel extends BaseWindowPanel {
	...
	protected onRegist(): void {
		...
		GameCommon.getInstance().addMsgEventListener(MESSAGE_ID.BOSS_MYSTERIOUS_HISTORY_TOP_MESSAGE.toString(), this.onResKillLogMsg, this);
	}
	protected onRemove(): void {
		super.onRemove();
		this.btn_sure.removeEventListener(egret.TouchEvent.TOUCH_TAP, this.onHide, this);
		GameCommon.getInstance().removeMsgEventListener(MESSAGE_ID.BOSS_MYSTERIOUS_HISTORY_TOP_MESSAGE.toString(), this.onResKillLogMsg, this);
	}
	//供子类覆盖
	protected onInit(): void {
		this.setTitle("cross_killlog_title_png");
	...
	private onRequestKillLogMsg(): void {
		var killlogMsg: Message = new Message(MESSAGE_ID.BOSS_MYSTERIOUS_HISTORY_TOP_MESSAGE);
	...
	public onHide(): void {
		...
		if (this.param.isLeave && GameFight.getInstance().fightsceneTpye == FIGHT_SCENE.MYSTERIOUS_BOSS) {
			GameDispatcher.getInstance().dispatchEvent(new egret.Event(GameEvent.GAME_CLOSE_RESULT_VIEW));
		}
	}
	//The end
}
class MyBossKillLogItem extends eui.ItemRenderer {
