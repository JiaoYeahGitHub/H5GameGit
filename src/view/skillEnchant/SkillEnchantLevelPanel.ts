class SkillEnchantLevelPanel extends BaseTabView {
	public powerBar: PowerBar;
	public curPro: eui.Label;
	public nextPro: eui.Label;
	...
	protected onUpdateAttribute(): void {
		...
		for (let i = 0; i < model.attrAry.length; ++i) {
			...
			if (attrValue > 0) {
				str = str + GameDefine.Attr_FontName[i] + "：" + (attrValue * level) + '\n';
				attr_ary[i] = attrValue * level;
				nextStr = nextStr + GameDefine.Attr_FontName[i] + "：" + (attrValue * (level + 1)) + '\n';
			}
		}
	...
	protected onUpdateCost(): void {
		...
		if (level == 0) {
			...
			// let costNum: number = 50000 + level * 2000;
			let costItem: AwardItem = GameCommon.parseAwardItem(model.jihuocost);
			this.consumItem.setCostByAwardItem(costItem);
	...
	protected onAdvanceHanlde(): void {
		...
		if (GameCommon.getInstance().onCheckItemConsume(0, costNum, GOODS_TYPE.YUELI)) {
